...
in
{
  options.services.darwin-vz = {
    ...
    user = lib.mkOption {
      type = lib.types.str;
      default = "root";
      description = "User who should have SSH access to the guest VM.";
    };
  };
    ...
    environment.etc."ssh/ssh_config.d/200-darwin-vz-nix.conf" = {
      text = ''
        Host darwin-vz-nix
          ...
          IdentityFile /Users/${cfg.user}/.ssh/darwin-vz-nix
          StrictHostKeyChecking accept-new
          UserKnownHostsFile /Users/${cfg.user}/.ssh/darwin-vz-nix_known_hosts
      '';
    };
    ...
    system.activationScripts.darwin-vz-nix = {
      text = ''
        ...
        if [ -f "${cfg.workingDirectory}/ssh/id_ed25519" ]; then
          USER_SSH_DIR="/Users/${cfg.user}/.ssh"
          mkdir -p "$USER_SSH_DIR"
          install -m 600 -o ${cfg.user} "${cfg.workingDirectory}/ssh/id_ed25519" "$USER_SSH_DIR/darwin-vz-nix"
          install -m 644 -o ${cfg.user} "${cfg.workingDirectory}/ssh/id_ed25519.pub" "$USER_SSH_DIR/darwin-vz-nix.pub"
        fi
      '';
    };
  };
