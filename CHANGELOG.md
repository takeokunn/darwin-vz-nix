...
- Guest `builder` user now has `linger` enabled. Every `nix.buildMachines` connection is its own short-lived SSH login; without lingering, systemd-logind SIGKILLs the guest's `user@<uid>.service` (and every process under it, including *other* still-active sessions' `nix-daemon --stdio`) the instant the last `builder` session ends. Confirmed live: every real distributed build died immediately with "interrupted by the user" on the guest, surfaced to the host as "Nix daemon disconnected unexpectedly" — this was blocking the darwin-vz-nix builder from completing any remote build at all
