...
### Fixed
- Self-hosted VM smoke and release-gate jobs now reuse the runner's managed Nix installation and configure public binary caches directly, avoiding the legacy Cachix CLI installer while authenticating cache pulls with pinned public keys
...
- **Local privilege escalation**: the root/launchd daemon's `known_hosts` scrub of the console user's `~/.ssh` no longer follows symlinks. `ssh-keygen -R` and the ownership restore are now guarded by an `O_NOFOLLOW` open (regular files only) and the chown goes through `fchown` on that descriptor, so a symlink swapped into the user-owned `~/.ssh` can no longer redirect the root chown/rewrite onto an arbitrary file (e.g. `/etc/sudoers`)
- **Disk-image corruption on concurrent start**: `start` now takes an exclusive, process-lifetime `flock` on `<state-dir>/vm.lock` before touching `disk.img`, so two `start`s racing on the same `--state-dir` can no longer both open the image read-write
- `console.log` is created mode 0600 (and its `newsyslog` rotation mode tightened to 0600) so guest boot output, which can contain build secrets, is not world-readable
- A preposterous `--memory` value no longer crashes with an arithmetic-overflow trap; the MB→bytes conversion saturates and is then clamped to the framework maximum
- **CI supply-chain hardening**: the `setup-nix` action no longer sets `accept-flake-config = true` (which would silently trust attacker-supplied `extra-substituters`/`trusted-public-keys` in a fork PR's `flake.nix`); trusted substituters are pinned explicitly in the action instead. `cachix-action` runs with `skipPush: true` so only the explicit, branch/tag-gated `cachix push` steps publish to the cache, and every `actions/checkout` uses `persist-credentials: false` so the job token is not left in `.git/config` during arbitrary Nix builds
...
- The `start` known-hosts scrub no longer leaves the console user's `~/.ssh/darwin-vz-nix_known_hosts` root-owned. The daemon runs as root, and `ssh-keygen -R` rewrites the file in place (plus a `.old` backup), so scrubbing the console user's file flipped it to `root:wheel` — after which that user's plain `ssh darwin-vz-nix` could no longer append the new host key ("Failed to add the host to the list of known hosts"). `start` now chowns the file back to the console user and removes the stray root-owned `.old` backup
...
[Unreleased]: https://github.com/takeokunn/darwin-vz-nix/compare/v0.2.2...HEAD
[0.2.2]: https://github.com/takeokunn/darwin-vz-nix/releases/tag/v0.2.2
